...
public class fs_A_day31_prgm2 {
 ...
 public static int get_max_cost(int [][]mat,int r,int c){
        ...
        for(int i=0;i<r;i++)
        {
            int cost=get_cost(mat,i,0,r,c,dp);
            max_c=Math.max(cost,max_c);
        }
        return max_c;
    }
    public static int get_cost(int [][]mat,int i,int j,int m,int n,int dp[][]){
        if((i<0) || (i==m) || j==n){
            return 0;
        }
        if(dp[i][j]!=-1)
        ...
        int right_path=get_cost(mat,i,j+1,m,n,dp);
        int right_upper_diagonal=get_cost(mat,i-1,j+1,m,n,dp);
        int right_lower_diagonal=get_cost(mat,i+1,j+1,m,n,dp);
        dp[i][j]= mat[i][j]+Math.max(Math.max(right_upper_diagonal,right_lower_diagonal),right_path);
        return dp[i][j];
